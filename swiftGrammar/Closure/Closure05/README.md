...
> 캡처된 값을 클로저 내에서 사용할 수 있도록 해주는데,

- **_Value Capture(복사해서 사용하는 방법)_**
- **_Reference Capture(참조를 유지하는 방법)_**

Value Capture할 때 사용하는 문법 -> **Caputure List**
...
Caputure List에서 쓴 arguments들이 참조타입이 아니라면 해당 요소들은 클로저가 생성될때 초기화 됨
...
1. a는 Caputure List, 클로저가 생성될때 값 0을 획득
2. b는 Caputrue List X
3. a, b의 값을 10으로 변경한 후 클로저를 실행하면 a는 클로저가 생성되었을때 캡쳐한 값 0을 갖음, 하지만 b는 변경된 값을 사용

> a는 클로저가 생성됨과 동시에 Caputure List 내에 다시 a라는 이름의 상수로 초기화한 것
...
'self' 를 클로저의 Capture List에 포함하여 암시적으로 self를 참조하는 것

즉 클로저가 reference capture를 수행하는 것을 의미
...
> self가 구조체나 열거형의 인스턴스인 경우 항상 암시적으로 self를 참조할 수 있음

그러나 이스케이프 클로저는 self가 Struct나 Enum의 인스턴스일 때 self에 대한 변경 가능한 참조를 캡처할 수 X
...
> Capture List에 강한 캡쳐(Strong Capture) , 약한 캡쳐(Weak Capture) , 미소유 캡쳐(Unowned Capture) 를 참조할 것인지 정해줄 수 있음

즉 클로저가 외부 변수를 참조할 때, 해당 변수의 수명과 관련된 문제를 생각해야함

이를 위해 클로저 캡처 capture list를 사용하여 변수의 참조 방식을 지정하는 것
...
1. 클로저가 강한 참조로 self를 캡처하므로 self의 인스턴스를 사용할 수 있음
...
1. weak 참조로 self를 캡처하므로 self의 인스턴스가 해제되어도 클로저가 실행됨.
   이는 클로저가 약한 참조로 self를 가지고 있기 때문에, self의 참조 횟수가 증가되지 않으며 클로저가 실행되는 동안에만 해당 인스턴스가 유지되는 것

### Unowned Capture(미소유 캡쳐)
...
1. 클로저가 미소유 참조로 self를 캡처하므로 self의 인스턴스가 해제되면 런타임 에러가 발생함

   > uObj의 참조 카운트가 0이 되는 시점에 메모리에서 해제될 경우
...
2. Weak Capture(약한 캡쳐)과 Unowned Capture(미소유 캡쳐)
   외부 변수의 참조 횟수를 증가시키지 않으므로, 클로저가 사용 중이더라도 변수가 해제될 수 있음.
   <br/>

   이러한 경우, 외부 변수가 이미 메모리에서 해제된 상태일 수 있으므로, 변수를 옵셔널로 선언하여 클로저가 변수를 사용할 때 nil 여부를 체크해야 함
   <br/>
...
## Weak Capture(약한 캡쳐)과 Unowned Capture(미소유 캡쳐) 주의할 점
...
uObj를 해제한 후에도 wObj의 참조는 여전히 남아 있는 경우

이는 uObj의 doSay 프로퍼티가 wObj의 doSay 클로저를 참조하고 있기 때문에 발생하는 문제

이로 인해 wObj가 해제되지 않고 메모리에 유지되며, 메모리릭가 발생

> 만약 미소유 참조로 문제가 발생 시 약한참조로 변경하고 옵셔널을 사용해도 무방
...
[블로그 참조](https://boidevelop.tistory.com/37)
